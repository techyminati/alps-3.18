...
#define COLOR_NLOG(fmt, arg...) DDPMSG("[COLOR] " fmt "\n", ##arg)
			    ...
			    void *__cmdq)
{
	...
	/* DDPMSG("==========config COLOR g_Color_Param [%d %d %d %d %d %d %d %d %d]\n===============\n",
		pq_param_p->u4SatGain, pq_param_p->u4HueAdj[PURP_TONE], pq_param_p->u4HueAdj[SKIN_TONE],
		pq_param_p->u4HueAdj[GRASS_TONE], pq_param_p->u4HueAdj[SKY_TONE],
		pq_param_p->u4SatAdj[PURP_TONE], pq_param_p->u4SatAdj[SKIN_TONE],
		pq_param_p->u4SatAdj[GRASS_TONE], pq_param_p->u4SatAdj[SKY_TONE]); */
	...
	for (index = 0; index < 8; index++) {
		_color_reg_set(cmdq, DISP_COLOR_Y_SLOPE_1_0_MAIN + 4 * index,
			       (g_Color_Index.
				PARTIAL_Y[2 * index] | g_Color_Index.PARTIAL_Y[2 * index +
									       1] << 16));
	}
	...
	case DISP_IOCTL_WRITE_REG:
		{
			...
			if (ret == 2) {
				if (cmdq == NULL) {
					...
					/* cmdqRecWrite(cmdq,
							TDSHP_PA_BASE + (wParams.reg - g_tdshp_va),
