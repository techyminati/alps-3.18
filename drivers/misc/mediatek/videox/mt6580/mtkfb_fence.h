...
/* use another struct to avoid fence dependency with ddp_ovl.h */
	...
	struct mtkfb_fence_buf_info *mtkfb_init_buf_info(struct
							 mtkfb_fence_buf_info
							 *buf);
	...
	void mtkfb_update_fence_set_input_ticket(unsigned int session_id,
						 ...
						 int enable);
	void mtkfb_update_present_fence_ticket(unsigned int session_id,
					       int fence, unsigned int ticket);
	...
	void mtkfb_release_layer_fence(unsigned int session_id,
				       unsigned int layer_id);
	...
	struct mtkfb_fence_buf_info *disp_sync_prepare_buf(disp_buffer_info *
							   buf);
