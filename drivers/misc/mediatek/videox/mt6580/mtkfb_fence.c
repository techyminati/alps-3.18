...
static disp_session_sync_info *_get_session_sync_info(unsigned int session_id)
{
	...
	for (i = 0;
	     i < sizeof(_disp_fence_context) / sizeof(_disp_fence_context[0]);
	     i++) {
	...
	for (i = 0;
		...
		if (_disp_fence_context[i].session_id == 0xffffffff) {
			...
			dprec_logger_event_init(&session_info->event_prepare,
						name,
			...
			sprintf(name, "%s%d_setinput",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_setinput,
						name,
						DPREC_LOGGER_LEVEL_DEFAULT |
						DPREC_LOGGER_LEVEL_SYSTRACE,
						&ddp_mmp_get_events()->session_Parent);
			...
			sprintf(name, "%s%d_setoutput",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_setoutput,
						name,
						DPREC_LOGGER_LEVEL_DEFAULT |
						DPREC_LOGGER_LEVEL_SYSTRACE,
						&ddp_mmp_get_events()->session_Parent);
			...
			sprintf(name, "%s%d_trigger",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_trigger,
						name,
						DPREC_LOGGER_LEVEL_DEFAULT |
						DPREC_LOGGER_LEVEL_SYSTRACE,
						&ddp_mmp_get_events()->session_Parent);
			...
			sprintf(name, "%s%d_findidx",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_findidx,
						name,
						DPREC_LOGGER_LEVEL_DEFAULT,
						&ddp_mmp_get_events()->session_Parent);
			...
			sprintf(name, "%s%d_release",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_release,
						name,
						DPREC_LOGGER_LEVEL_DEFAULT |
						DPREC_LOGGER_LEVEL_SYSTRACE,
						&ddp_mmp_get_events()->session_Parent);
			...
			sprintf(name, "%s%d_waitvsync",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_waitvsync,
						name,
						DPREC_LOGGER_LEVEL_DEFAULT |
						DPREC_LOGGER_LEVEL_SYSTRACE,
						&ddp_mmp_get_events()->session_Parent);
...
disp_session_sync_info *disp_get_session_sync_info_for_debug(unsigned int
							     session_id)
{
	return _get_session_sync_info(session_id);
}

disp_sync_info *_get_sync_info(unsigned int session_id,
			   ...
			   unsigned int seq)
{
	...
	list_for_each_entry(buf, &layer_info->buf_list, list) {
		if (buf->idx == idx) {
			...
			buf->seq = seq;
			ret = true;
			MMProfileLogEx(ddp_mmp_get_events()->primary_seq_insert,
				       MMProfileFlagPulse,
...
struct mtkfb_fence_buf_info *mtkfb_init_buf_info(struct mtkfb_fence_buf_info
						 *buf)
{
...
static struct mtkfb_fence_buf_info *mtkfb_get_buf_info(void)
{
	...
	if (!list_empty(&info_pool_head)) {
		info =
		    list_first_entry(&info_pool_head,
	...
	if (layer_info->timeline != NULL) {
		...
		list_for_each_entry_safe(buf, n, &layer_info->buf_list, list) {
			if (buf->idx <= fence) {
				...
				DISPPR_FENCE
				    ("R+/%s%d/L%d/id%d/last%d/new%d/free/idx%d/mva0x%lx\n",
				     disp_session_mode_spy(session_id),
				     DISP_SESSION_DEV(session_id), layer_id,
...
static int prepare_ion_buf(disp_buffer_info *buf,
