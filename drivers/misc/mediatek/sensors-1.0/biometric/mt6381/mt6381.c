				       ...
				       u32 *len)
{
	...
	*len = sram_len;
	...
	if (sram_len > 0) {
		...
		if (atomic_read(&bio_trace) != 0) {
			BIOMETRIC_LOG("%s =====> sram_type: %d, sram_len,%d,%d,%d\n",
...
static enum vsm_status_t vsm_driver_read_offline_mode_data(enum vsm_signal_t type, uint32_t *data_buf, u32 *len)
{
	...
	*len = 0;
	...
	current_time = (current_time - olm_info_p->en_time) * 512;
	do_div(current_time, 1000000000);
	target_num = current_time;
...
static int MT6381_DoCalibration(struct biometric_cali *cali)
{
	...
	pr_err("%s, %d, %d, %d\r\n", a, b, c);
	sumOfambdac = sumOfambdac * -1000;
	do_div(sumOfambdac, count);
	cali->ambdac5_5 = sumOfambdac;
	if (atomic_read(&bio_trace) != 0) {
